...
/* ══════════════ SUPABASE ══════════════ */
const SUPA_URL = "https://nqkitzmprepugyxkfcke.supabase.co";
const SUPA_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im5xa2l0em1wcmVwdWd5eGtmY2tlIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQwMDExNDksImV4cCI6MjA4OTU3NzE0OX0.MmDlM2dPSJpSqTr92BcGbe0EsXWpH8hBP0dDGlT38mQ";
const AI_FN_URL = `${SUPA_URL}/functions/v1/ai-chat`;
...
const supa = {
  ...
  async aiChat(token,messages,system){
    const r=await fetch(AI_FN_URL,{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Bearer ${token}`},body:JSON.stringify({messages,system})});
    return r.json();
  },
};
...
const DARK = {
  ...
  text:"#ECEEF6", mid:"#A0A0C0", light:"#6A6A8A", border:"#2E2E44",
  shadow:"rgba(0,0,0,0.35)",
  pink:"#7A3040", pinkBg:"#2A1520",
  lav:"#4A4278",  lavBg:"#1E1A38",
  mint:"#1E5040", mintBg:"#0E2820",
  peach:"#6A4828",peachBg:"#2A1C0E",
  red:"#E05060", green:"#3DBF80", blue:"#4E8EE0", orange:"#F09050",
};
...
function SignIn({onSuccess,onSignUp,onForgot}){
  ...
  const submit=async()=>{
    ...
    try{
      ...
      setErrs({pw:"Connection error. Try again."});
    }
    setLoading(false);
  };
  ...
  return <div className="auth-wrap">
    ...
    <div style={{flex:1,paddingTop:16,maxWidth:480,margin:"0 auto",width:"100%",padding:"16px 24px 40px",boxSizing:"border-box"}}>
      ...
      <div style={{display:"flex",gap:10,marginBottom:28}}>
        <button className="btn-social" onClick={()=>alert("Google Sign-In coming soon! Use email & password for now.")} style={{flex:1}}>{Ic.google}Google</button>
        <button className="btn-social" onClick={()=>alert("Apple Sign-In coming soon! Use email & password for now.")} style={{flex:1}}>{Ic.apple}Apple</button>
      </div>
...
function SignUp({onSuccess,onSignIn}){
  ...
  const next=async()=>{
    ...
    try{
      ...
      setErrs({submit:err?.message||"Connection error. Check your internet and try again."});
      setLoading(false);
    }
  };
  ...
  return <div style={{paddingBottom:110}}>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"2px 16px 16px",maxWidth:600,margin:"0 auto",width:"100%"}}>
      <div style={{display:"flex",alignItems:"center",gap:11}}>
        <div className="avatar" style={{width:44,height:44,fontSize:17,boxShadow:`0 3px 10px ${T.shadow}`}}>{user.name?.[0]?.toUpperCase()}</div>
        <div><p style={{fontSize:12,color:T.light,fontWeight:600,lineHeight:1}}>Hello,</p><p style={{fontSize:18,fontWeight:900,lineHeight:1.3}}>{user.name}! 👋</p></div>
      </div>
      <div style={{display:"flex",gap:9}}>
        <button className="btn-icon" onClick={showSearch}>{Ic.search}</button>
        <button className="btn-icon" onClick={showAwards} style={{position:"relative"}}>{Ic.trophy}</button>
    ...
    <div style={{padding:"0 16px",display:"flex",flexDirection:"column",gap:14,maxWidth:600,margin:"0 auto",width:"100%"}}>
      ...
      {/* Activity + weight */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        <div className="card hover-card" style={{padding:"16px"}} onClick={()=>setSheet("sport")}>
          <p style={{fontSize:13,fontWeight:800,marginBottom:4}}>My Activity Recaps</p>
          <p style={{fontSize:11,color:T.light,lineHeight:1.4,fontWeight:500,marginBottom:10}}>Everything you need to know.</p>
          ...
          <button className="btn btn-primary" style={{marginTop:10,borderRadius:12,padding:"10px 14px",fontSize:13}} onClick={e=>{e.stopPropagation();setSheet("sport");}}>Get Started</button>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
      ...
      <div>
        ...
        <div style={{display:"flex",gap:12,overflowX:"auto",paddingBottom:4}}>
          {RECS.slice(0,4).map((r,i)=><div key={i} className="hover-card" style={{minWidth:128,padding:"16px",borderRadius:20,background:r.bg,flexShrink:0,boxShadow:`0 3px 10px ${T.shadow}`}} onClick={()=>addRec(r)}>
            ...
            <p style={{fontSize:14,fontWeight:800}}>{r.n}</p>
            <p style={{fontSize:12,color:T.mid,fontWeight:600,marginTop:3}}>{r.c} kcal</p>
            <div style={{fontSize:10,color:T.purple,fontWeight:700,marginTop:6}}>Tap to add +</div>
          </div>)}
        </div>
      </div>
      {/* Bottom sport+hydration */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:4}}>
        <div className="pcard hover-card" style={{background:T.peachBg,padding:"16px"}} onClick={()=>setSheet("sport")}><div style={{display:"flex",justifyContent:"space-between"}}><div style={{width:32,height:32,borderRadius:9,background:T.white,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>📊</div><div style={{color:T.mid}}>{Ic.trend}</div></div><p style={{fontSize:14,fontWeight:800,marginTop:10}}>Sport Data</p><p style={{fontSize:11,color:T.mid,fontWeight:500,marginTop:3,lineHeight:1.4}}>Keep Active, Keep Healthy</p><button className="check-link" style={{marginTop:12}}>Check {Ic.arrowR}</button></div>
...
function Scanner({onAddMeal,toast,user,online=true}){
  ...
  const searchFood=async(q)=>{
    ...
    try{
      // Try Indian + global search
      const url=`https://world.openfoodfacts.org/cgi/search.pl?search_terms=${encodeURIComponent(q)}&search_simple=1&action=process&json=1&page_size=40&fields=product_name,brands,nutriments,serving_size,categories_tags,image_front_small_url,nutriscore_grade`;
  ...
  const lookupBarcode=async(code)=>{
    ...
    }catch(e){toast("Network error.","❌");setBarcodeStatus("idle");}
  ...
  const startBarcodeCam=async()=>{
    ...
    }catch(e){setCamErr("Camera access denied.");setBarcodeStatus("idle");}
  };
  ...
  const doAiScan=async()=>{
    ...
    try{
      ...
      const r=await fetch(`${SUPA_URL}/functions/v1/ai-chat`,{
        method:"POST",
        headers:{"Content-Type":"application/json","Authorization":`Bearer ${user?.token||""}`},
        body:JSON.stringify({
          system:"You are a food recognition AI. Analyze the image and identify the food. Respond ONLY with valid JSON in this exact format: {\"name\":\"Food Name\",\"cal\":number,\"p\":number,\"c\":number,\"f\":number,\"emoji\":\"emoji\",\"items\":[\"ingredient 1\",\"ingredient 2\"]} where cal=calories, p=protein(g), c=carbs(g), f=fat(g). Estimate for a standard serving. No extra text.",
          messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:"image/jpeg",data:base64}},{type:"text",text:"What food is this? Give me the nutrition info as JSON."}]}],
        }),
      });
      const data=await r.json();
      ...
      if(match){
        ...
        toast("AI couldn't identify. Try manual entry.","❌");
      }
    }catch(e){toast("AI scan failed. Try again.","❌");}
...
/* ══════════════ SETTINGS SCREEN ══════════════ */
function Settings({user,setUser,toast,onSignOut,settings,setSetting}){
  const [sheet,setSheet]=useState(null);
  ...
  return <div style={{paddingBottom:110}}><SBar/>
    <div style={{padding:"4px 16px 20px",maxWidth:600,margin:"0 auto",width:"100%"}}>
      ...
      <div className="pcard" style={{background:T.lavBg,marginBottom:16,cursor:"pointer"}} onClick={()=>{setEditForm(user);setSheet("profile");}}>
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          <div className="avatar" style={{width:56,height:56,fontSize:22,boxShadow:`0 4px 12px ${T.shadow}`}}>{user.name?.[0]?.toUpperCase()}</div>
          <div style={{flex:1}}>
      ...
      <div className="card" style={{marginBottom:14}}>
        ...
        <SettingRow icon={<span style={{fontSize:17}}>🌐</span>} iconBg={T.peachBg} label="Language" sub={lang} onClick={()=>setSheet("language")}/>
      </div>
      ...
      <div className="card" style={{marginBottom:14}}>
        ...
        <SettingRow icon={Ic.export} iconBg={T.lavBg} label="Export Data" sub="PDF or CSV" onClick={()=>setSheet("export")}/>
      ...
      <div className="card" style={{marginBottom:16}}>
        ...
        <SettingRow icon={<span style={{fontSize:17}}>ℹ️</span>} iconBg={T.bg} label="App Version" sub="NutriScan v2.1.0" right={<span className="badge" style={{background:T.mintBg,color:T.green,fontSize:11}}>Latest</span>}/>
      </div>
    ...
    {sheet==="export"&&<Sheet title="📤 Export Data" onClose={()=>setSheet(null)}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {[{t:"📄 PDF Report",s:"Full formatted report with charts",bg:T.pinkBg,fn:"PDF"},{t:"📊 CSV Spreadsheet",s:"Raw data for Excel / Google Sheets",bg:T.mintBg,fn:"CSV"},{t:"🗂️ JSON Export",s:"Complete data backup",bg:T.lavBg,fn:"JSON"}].map(x=><button key={x.fn} onClick={()=>{toast(`${x.fn} download started!`,Ic.export);setSheet(null);}} style={{width:"100%",padding:"16px",borderRadius:16,background:x.bg,border:"none",cursor:"pointer",textAlign:"left",fontFamily:"Nunito",transition:"all .18s"}} onMouseEnter={e=>e.currentTarget.style.filter="brightness(.96)"} onMouseLeave={e=>e.currentTarget.style.filter="none"}><p style={{fontSize:15,fontWeight:800}}>{x.t}</p><p style={{fontSize:12,color:T.mid,marginTop:4}}>{x.s}</p></button>)}
      </div>
    </Sheet>}
    ...
    {/* Language */}
    {sheet==="language"&&<Sheet title="🌐 Language" onClose={()=>setSheet(null)}>
      {["English","हिन्दी","தமிழ்","తెలుగు","বাংলা","मराठी","Español","Français"].map(l=><div key={l} onClick={()=>{setLang(l);toast(`Language set to ${l}`,"🌐");setSheet(null);}} style={{padding:"14px",borderRadius:14,cursor:"pointer",background:lang===l?T.lavBg:T.bg,border:`2px solid ${lang===l?T.purple:"transparent"}`,display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8,fontWeight:lang===l?800:600,transition:"all .18s"}}><span>{l}</span>{lang===l&&<div style={{color:T.purple}}>{Ic.check}</div>}</div>)}
    </Sheet>}
    ...
    {sheet==="feedback"&&<Sheet title="💬 Send Feedback" onClose={()=>setSheet(null)}>
      ...
      <div style={{marginBottom:14}}><label className="flabel">What's on your mind?</label><textarea className="inp" rows={5} placeholder="Tell us what you love, what's missing, or what can be improved…" style={{resize:"none",lineHeight:1.6}}/></div>
      <div style={{display:"flex",gap:10}}>
        <button className="btn btn-ghost" style={{flex:1,border:"2px solid "+T.border}} onClick={()=>setSheet(null)}>Cancel</button>
        <button className="btn btn-primary" style={{flex:1}} onClick={()=>{toast("Feedback sent! Thanks 🙏","💬");setSheet(null);}}>Send Feedback</button>
      </div>
    </Sheet>}
    ...
    {sheet==="clearData"&&<ModalBox title="⚠️ Clear All Data" onClose={()=>setSheet(null)}>
      ...
      <div style={{display:"flex",gap:10}}>
        ...
        <button className="btn btn-danger" style={{flex:1}} onClick={()=>{toast("All data cleared","🗑️");setSheet(null);}}>Delete Everything</button>
      </div>
    </ModalBox>}
...
function AiChat({user,meals,toast,online=true}){
  ...
  const send=async()=>{
    ...
    try{
      ...
      const data=await supa.aiChat(user?.token,newMsgs.map(m=>({role:m.role,content:m.text})),system);
      ...
      setMessages(p=>[...p,{role:"assistant",text:"Oops! Something went wrong. Please try again. 🙏"}]);
    }
...
export default function App(){
  ...
  const [settings,setSettings]=useState({mealReminders:true,waterReminders:true,healthInsights:true,achievementAlerts:true,darkMode:false,offline:true});
  ...
  useEffect(()=>{
    const restore=async()=>{
      ...
      if(!s?.token)return;
      try{
        const u=await supa.getUser(s.token);
        const uid=u.id||u.user?.id;
        if(u.error||!uid)return clearSession();
        const profile=await supa.select(s.token,"profiles","*",`&id=eq.${uid}`);
        const p=Array.isArray(profile)&&profile[0];
        if(!p)return clearSession();
        const authedUser={token:s.token,id:uid,name:p.name,email:u.email||p.email,age:String(p.age||28),gender:p.gender||"male",height:String(p.height||170),weight:String(p.weight||70),activity:p.activity||"moderate",goal:p.goal||"loss"};
        setUser(authedUser);
        setScreen("app");
        await loadUserData(s.token,uid);
  ...
  const setMealsAndSave=async(updater)=>{
    ...
    for(const m of newMeals){
      ...
      try{await supa.insert(user.token,"meals",payload);}catch(e){enqueueOffline({kind:"meal",payload});}
    }
  };
  ...
  return <>
    ...
    {splash&&<SplashScreen/>}
    <div className="shell" style={{background:T.bg,color:T.text,opacity:splash?0:1,transition:"opacity .4s"}}>
      {screen==="landing" && <Landing onSignIn={()=>setScreen("signin")} onSignUp={()=>setScreen("signup")}/>}
      {screen==="signin"  && <SignIn  onSuccess={handleAuth} onSignUp={()=>setScreen("signup")} onForgot={()=>setScreen("forgot")}/>}
      {screen==="signup"  && <SignUp  onSuccess={handleAuth} onSignIn={()=>setScreen("signin")}/>}
      {screen==="forgot"  && <ForgotPassword onBack={()=>setScreen("signin")}/>}
      {screen==="app"&&user&&<>
        ...
        <div style={{height:"100vh",overflowY:"auto",width:"100%"}}>
          ...
          {tab==="settings"     && <Settings     user={user} setUser={setUser} toast={(m,i)=>toast(m,i||"✅")} onSignOut={signOut} settings={settings} setSetting={setSetting}/>}
        </div>
